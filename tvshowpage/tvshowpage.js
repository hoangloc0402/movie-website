const movie_name = "Grey's Anatomy - Season 8"
const src = "images/1.png";
const released_date = "2011";
const imdb = 7.7;
const tag = [
	{g:"Drama",s:"#"},
	{g:"Romance",s:"#"}
]
const description = "In this season, challenges keep coming. Loyalties will be tested, relationships will be strained and everyone’s future will hang in the balance.But no matter how hard life is, the doctors of Seattle Grace know they can always lean on one another."
const list_ep = [
	{
		href : "#",
		name : "Flight",
...
(function($, viewport){
	...
	function render_slide() {
		...
		if (img_movie_rendered === false) {
			for (var i = list_ep.length - 1; i >= list_ep.length - 2; i --) {
				...
				wrapper.append(div,
					`<span class="movie-title">${movie_name + " Episode " + list_ep[i].ep + ": " + list_ep[i].name}</span>`)
		...
		if (img_movie_rendered === false) {
			...
			for (var i = list_ep.length - 1; i >= 0; i --) {
				var div =  $(`<div class="movie-thumnail"></div>`)
				div.css('background-image', 'url(' + src + ')');
				div.append(`<span class="movie-play-button"></span>`)
				$(div).width(card_width);
    ...
    $(document).ready(function() {
			...
			$("#movies_name_div").text(movie_name);
			$("#movies_poster").attr("src",src);
			$("#movies_released_date").append(released_date);
			$("#movies_imdb").append(imdb);
			$("#movies_desp").text(description);
			for (var i = 0; i < tag.length; i++) {
				if (i == 0)
					$("#movies_tags").append(`<a href=${tag[i].s}>${tag[i].g}</a>`)
				else 
					$("#movies_tags").append(` / <a href=${tag[i].s}> ${tag[i].g}</a>`)
			}
			render_slide();
